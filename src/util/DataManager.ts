...
export default class DataManager {
	static async getData(key: String, skip?: Number, filter?: { [key: string]: string }) {
	...
	static async getCollection(collection: String, id: String) {
		if (id) {
			// [TODO]
	...
	static async getEntry(collection: String, id: String) {
		if (id) {
			// [TODO]
			// Switch query for every collection

			const params = {
				city: "{ name, country { name, sys { id } }, galleryCollection { items { title, url } }, coordinates { lat, lon }, description { json }, sys { id } }",
				place:
					"{ name, city { name, country { name, sys { id } }, sys { id } }, galleryCollection { items { title, url } }, coordinates { lat, lon }, description { json }, schedule { json }, sys { id } }",
			};

			// @ts-ignore
			const query = `{ ${collection}(id: "${id}") ${params[collection]} }`;
			const response = await window.fetch(
				`https://graphql.contentful.com/content/v1/spaces/${contentful.spaceID}/`,
				{
					method: "POST",
